...
<script>
  $(document).ready(function() {
    $("#signInButton1").click(function(e) {
      e.preventDefault();
      var username = $("#userInputEmail1").val().trim();
      var password = $("#userInputPassword1").val().trim();
      if (username && password) {
        $.ajax({
          ...
          success: function(response) {
            if(response.success){
            // $("#error_message").text(response.success).css("color", "green");
            window.location.href = "index.php";
            }

            else{
              $("#error_message").text(response.error).css("color", "red");
            }
          },
        ...
        $("#error_message").text("Username and password cannot be empty").css("color", "red");
      }
    });
